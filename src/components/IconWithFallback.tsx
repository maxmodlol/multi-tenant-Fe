...
// Fallback SVG icons for common social media platforms
const FallbackIcons = {
  WhatsApp: (
...
export default function IconWithFallback({
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (isLoading) {
        console.warn(`Icon loading timeout: ${src}`);
        setHasError(true);
        setIsLoading(false);
      }
  ...
  const handleError = () => {
    ...
    console.warn(`Icon path resolution: ${src}`);
    setHasError(true);
    setIsLoading(false);
  };
  ...
  if (hasError) {
    // Try to match the icon name to our fallback SVGs
    const iconName = alt || src.split("/").pop()?.split(".")[0] || "";
    const fallbackIcon = FallbackIcons[iconName as keyof typeof FallbackIcons];

    return (
      <div className={`flex items-center justify-center ${className}`} style={{ width, height }}>
        {fallbackIcon ? (
          <div className="text-gray-600 dark:text-gray-300">{fallbackIcon}</div>
        ) : fallbackText ? (
